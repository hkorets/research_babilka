...
```{r}
# 1. Group by year and calculate total gross income per year
total_gross_income_per_year <- movies %>%
  group_by(year) %>%
  summarise(TotalGrossIncome = sum(gross_income)) %>%
  arrange(desc(TotalGrossIncome))  # Sort in descending order

# 2. Select the top 15 years with highest gross income
top_15_years <- head(total_gross_income_per_year, 15)

# 3. Plot the results using a bar plot
...
top_movies_by_votes <- movies %>%
  arrange(desc(votes)) %>%
  head(15)  # Adjust the number (15) to any other number as needed

# Create a bar chart to visualize top movies by votes
ggplot(top_movies_by_votes, aes(x = reorder(name, votes), y = votes, fill = name)) +
  geom_bar(stat = "identity", color = "black", alpha = 0.7) +
  labs(title = "Top 15 Movies by Number of Votes",
       x = "Movie Name",
       y = "Number of Votes") +
  coord_flip() +  # Flip the coordinates to make the movie names readable
  theme_minimal() +
